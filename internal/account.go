...
func (db AccountQueryEngine) GetAccount(id uuid.UUID) (*Account, error) {
	stmt := `SELECT "id", "username", "email", "hashed_pass" FROM "accounts" WHERE "id" = $1;`
	if row := db.QueryRow(stmt, id); row != nil {
		var (
			id         string
			username   string
			email      string
